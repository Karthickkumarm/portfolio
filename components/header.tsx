...
export default function Header() {
  ...
  return (
    ...
    <motion.div className="fixed top-0 left-1/2 h-[4.5rem] -translate-x-1/2 w-full rounded-none border
     border-white border-opacity-40 bg-white bg-opacity-80 shadow-lg shadow-black/[0.03]
    backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.23rem] sm:w-[36rem] sm:rounded-full
    ...
    sm:flex-nowrap sm:gap-5 ">
        {links.map((link) => (
            ...
            animate={{y:0,opacity:1}}>
                <Link  className={clsx("flex w-flex items-center justify-center px-3 py-3  hover:text-gray-950 transition dark:text-gray-500 dark:hover:text-gray-300",
                    ...
                    {link.name}
                  {
                    link.name === activeSection && (
                      <motion.span className="bg-gray-100 rounded-full absolute
